...
void testDecOut(void){
	for(int32_t k = -100; k < 100; k += 1){
		ST7735_sDecOut3(k);
		fputc('\n',(FILE*)1);
	}
}
void testDecOutInvalids(void){
	ST7735_OutString("Dec out invalids");
	for(int32_t k = 10010;k >=9999; k -= 1){
		ST7735_sDecOut3(k);
		fputc('\n',(FILE*)1);
	}
	for(int32_t k = -10010;k <=-9999; k += 1){
	...
	for(int32_t k = 2147483647; k >= 2147483647 - 10; k -= 1){
		ST7735_sDecOut3(k);
		fputc('\n',(FILE*)1);
	}
	for(int32_t k = -2147483646; k <= -2147483646 + 10; k += 1){
		ST7735_sDecOut3(k);
		fputc('\n',(FILE*)1);
	}
}
void testBinOutCalc(void){
	ST7735_uBinOut8(0);
	fputc('\n',(FILE*)1);
	for(int32_t k = 1; k <= 262144; k *= 2){
		ST7735_uBinOut8(k);
		fputc('\n',(FILE*)1);
	}
}
void testBinOutBoundary(void){
	for(int32_t k = 255995; k < 256005; k += 1){
		ST7735_uBinOut8(k);
		fputc('\n',(FILE*)1);
	}
	ST7735_uBinOut8(-1);
...
void testBinOutInvalid(void){
	
}
void testInitPlot(void){
	ST7735_XYplotInit("Title", 0, 0, 0, 0);
}
void testPlotPrint(void){
	ST7735_XYplotInit("Test", 0, 127, 0, 127);
	int32_t X_arr[10];
	int32_t Y_arr[10];
	for (int32_t k = 12; k < 128; k += 12){
		X_arr[k] = k;
	  Y_arr[k] = k;
	}
	ST7735_XYplot(128, X_arr, Y_arr);
}
int mainA(void){ //mainA
...
int mainB(void){ //test decOut mainB
	PLL_Init(Bus80MHz);                  // set system clock to 80 MHz
  ST7735_InitR(INITR_REDTAB);
	testDecOutInvalids();
	testDecOut();
...
int mainC(void){ //mainC test binOut calculations
	PLL_Init(Bus80MHz);                  // set system clock to 80 MHz
  ST7735_InitR(INITR_REDTAB);
	testBinOutCalc();
	testBinOutBoundary();
...
int mainD(void){ //mainD test that plot init clears the screen
	PLL_Init(Bus80MHz);                  // set system clock to 80 MHz
  ST7735_InitR(INITR_REDTAB);
	testDecOutInvalids();
	testInitPlot();
...
int mainE(void){//mainE test that plot works
	PLL_Init(Bus80MHz);                  // set system clock to 80 MHz
  ST7735_InitR(INITR_REDTAB);
	testPlotPrint();
	return 0;
...
// Runs on TM4C123
// Uses ST7735.c LCD.
// Jonathan Valvano - Edited by Paul Cozzi and Eric Li
// August 31, 2016
